...
namespace ConsoleClient
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            Console.WriteLine($"InMemoryByteCopy() done. Total bytes: {InMemoryByByteCopy(source, destination)} (4 bytes lost on byte order mark)");
            Console.WriteLine(IsContentEquals(source, destination));
            ...
            Console.WriteLine($"ByLineCopy() done. Total lines: {ByLineCopy(source, destination)} (4 bytes lost on byte order mark)");
            Console.WriteLine(IsContentEquals(source, destination));
